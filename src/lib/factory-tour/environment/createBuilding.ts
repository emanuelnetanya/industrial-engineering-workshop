...
export function createBuilding(scene: BABYLON.Scene): void {
  ...
  const leftWall = BABYLON.MeshBuilder.CreateBox(
    'leftWall',
    { width: 1.5 * SCALE, height: wallHeight, depth: 100 },
    scene
  );
  leftWall.position = new BABYLON.Vector3(-42 * SCALE, wallHeight / 2, 0);
  ...
  rightWall.position = new BABYLON.Vector3(42 * SCALE, wallHeight / 2, 0);
  rightWall.material = wallMat;

  // קיר אחורי
  const backWall = BABYLON.MeshBuilder.CreateBox(
    'backWall',
    { width: 84 * SCALE, height: wallHeight, depth: 1.5 },
    scene
  );
  backWall.position = new BABYLON.Vector3(0, wallHeight / 2, 35 * SCALE);
  backWall.material = wallMat;

  // קיר קדמי עם שער כניסה
  const frontWallLeft = BABYLON.MeshBuilder.CreateBox(
    'frontWallLeft',
    { width: 30 * SCALE, height: wallHeight, depth: 1.5 },
    scene
  );
  frontWallLeft.position = new BABYLON.Vector3(-27 * SCALE, wallHeight / 2, -35);
  ...
  frontWallRight.position = new BABYLON.Vector3(27 * SCALE, wallHeight / 2, -35);
  ...
  for (const side of [-1, 1]) {
    ...
    pillar.position = new BABYLON.Vector3(side * 12 * SCALE, wallHeight / 2, -35);
  ...
  const roof = BABYLON.MeshBuilder.CreateBox(
    'roof',
    { width: 84 * SCALE, height: 0.8, depth: 100 },
    scene
  );
  ...
  // חלונות
  createWindows(scene, wallHeight, SCALE);
}

/**
 * יוצר חלונות בקירות
 */
function createWindows(scene: BABYLON.Scene, wallHeight: number, SCALE: number): void {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    window.position = new BABYLON.Vector3(-42 * SCALE, 10, -30 * SCALE + i * 15 * SCALE);
